...
def auto_recall_responses():
    """Send command responses through the plugin's auto-recall handler."""
    ...
    def decorator(func):
        ...
        async def wrapper(self, event, *args, **kwargs):
            ...
            async for result in func(self, event, *args, **kwargs):
                handled = await self.send_command_result_with_recall(
                    event,
                    result,
                    recall_source_message=recall_source_message,
                )
                if handled:
                    recall_source_message = False
                    continue
                yield result
